...
void FatalError(u4 errorcode, const u1* msg, ...);
...
u4 FS_WriteFmtb(FS* f, const void* str, ...);

u4 FS_Writeb(FS* f, const void* buff, u8 size);
...
u1* LoadFile(const u1* dir) {
	HANDLE h = CreateFileA((LPCSTR)dir, GENERIC_READ, 0, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
	if ((s8)h == (s8)(-1)) return 0;
	u4 size = 0;
...
void FatalError(u4 errorcode, const u1* msg, ...) {
	...
	u1 buff[TEMP_BUFF_CAP];
	u4 wb = 0;
	WriteFile(STDOUT->h, buff, CstrFmtS(buff, (u1*)msg, stack), (void*)&wb, 0);
...
u4 FS_Writeb(FS* fs, const void* buff, u8 size) {
	u8 p1 = STREAM_BUFF_CAP - fs->si;
	u4 writtenbytes = 0;
	if (p1 < size) {
		memcpy(fs->sb + fs->si, buff, p1);
		writtenbytes = WriteFile(fs->h, fs->sb, STREAM_BUFF_CAP, 0, 0);
...
u4 FS_WriteFmtb(FS* fs, const void* str, ...) {
	VAR_ARG(str, stack);
	u1 buff[TEMP_BUFF_CAP];
	return FS_Writeb(fs, buff, CstrFmtS(buff, (u1*)str, stack));
}
