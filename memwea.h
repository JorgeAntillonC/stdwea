...
u1 PtrRangeIsValid(u1* ptr, u4 size);
#else
...
u1 PtrRangeIsValid(u1* ptr, u4 size){
	...
	s4 readable = PAGE_READONLY | PAGE_READWRITE | PAGE_WRITECOPY | PAGE_EXECUTE_READ | PAGE_EXECUTE_READWRITE | PAGE_EXECUTE_WRITECOPY;
	if ((mbi.Protect & readable) == 0) return 0;

	u1* end = ptr + size;
	u1* region_end = (u1*)mbi.BaseAddress + mbi.RegionSize;
	if (end < ptr || end > region_end) return 0;
	return 1;
}
