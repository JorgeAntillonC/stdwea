...
u8 Cstr2Int(u1* str, u4 base);

u8 Cstr2IntPrefix(u1* str);

u8 CstrParseNum(u1* str, u4* len);

u8 CstrFromRawBytes(u1* str, u4 strlen, u1* buff, u4 bufflen);
...
inline u1 CstrIsNum(u1* str);
inline u1 CstrIsNumX(u1* str);
...
u8 Cstr2Int(u1* str, u4 base) {
	u8 v = 0;
	switch (base) {
		case 0b10: { while ((u1)(*str - '0') < 2)  { v = (v << 1) + *str++ - '0'; } } break;
		case  010: { while ((u1)(*str - '0') < 8)  { v = (v << 3) + *str++ - '0'; } } break;
		case   10: { while ((u1)(*str - '0') < 10) { v = (v * 10) + *str++ - '0'; } } break;
		case 0x10: {
...
// TODO check the bounds of bufflen properly
u8 CstrFromRawBytes(u1* str, u4 strlen, u1* buff, u4 bufflen) {
	u1* whead = buff;
	for (u4 i = 0; i < strlen && i < bufflen; i++) {
		if (CharIsPrintable(*str)) { *whead++ = *str++; }
		else {
			*whead++ = '\\';
			switch (*str) {
				case '\0': { *whead++ = '0'; } break;
				case '\n': { *whead++ = 'n'; } break;
				case '\r': { *whead++ = 'r'; } break;
				default: {
					*whead++ = 'x';
					const u1 hextable[] = "0123456789ABCDEF";
					*whead++ = hextable[(*str >> 4) & 0xF];
					*whead++ = hextable[*str & 0xF];
				} break;
			}
			str++;
		}
			...
			if (*++str != '%') {
				...
				switch (*str) {
					...
					case 'l':{
						...
						u1 pad = *str++;
						u1* b = buff + w;
						const s4 diff = (u4)ps.i - w;
						if (diff > 0) {
							for (u4 i = 0; i < diff; i++) { *b++ = pad; }
						}
						w = (u4)ps.i;
					}break;
				}
...
// for exact matches size should be the maximum size possible
u1 CstrCmpS(u1* str1, u1* str2, u4 size) {
...
inline u1 CstrIsNum(u1* str) {
	while (*str && CharIsNum(*str)) { str++; }
	return *str == '\0';
}

inline u1 CstrIsNumX(u1* str) {
	while (*str && CharIsNumX(*str)) { str++; }
	return *str == '\0';
}
...
inline u1 CharIsNumO(u1 c){ return (u1)(c - '0') < 8; }
